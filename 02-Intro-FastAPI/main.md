...
@app.get("/")
def home():
    return {"mensaje": "Bienvenidos a Mini Blog"} @app.get("/posts")  sin query para def list_post(     return {"data": BLOG_POST}
# PARA QUERYPARAMS
@app.get("/posts")
...
# ----------------------------------------------- En este archivo has creado un endpoint GET (/posts) que simula datos devolviendo una lista de post Esto es útil para pruebas, prototipos o cuando aún no tienes una base de datos rea
# ¿Qué más podrías hacer con este endpoin - Devolver datos más complejos: listas, diccionarios, objetos, et - Recibir parámetros en la URL (query o path parameters) para filtrar, buscar o paginar resultado - Validar y documentar los datos de entrada y salida usando Pydanti - Proteger el endpoint con autenticación o permiso - Registrar logs, manejar errores personalizados, et Ejemplo de endpoint GET con parámetro @app.get("/posts/{post_id} def get_post(post_id: int):
     for post in BLOG_POST:
         if post["id"] == post_id:
             return {"data": post}
     return {"error": "Post no encontrado"}
...
# -------------------------------------------------
 Para filtrar resultados en un endpoint GET, puedes usar parámetros de consulta (query params). Ejemplo de endpoint con filtro por query param:
```python
from fastapi import Query
 @app.get("/posts/search")
